...
- Comparison of pre-trained GPT with randomized parameters:
  - Both models perfom about the same (pretrained performs slightly better, but not significantly).
- Linguistic analysis:
  - Embeddings generated by perception do not appear to be grammatical.
  - Embeddings appear to use the same subspace as GPT's pretrained embeddings (if they are visualized using dimensionality reduction, the perception embeddings overlap with the cloud of GPT embeddings).


### Goals set last time
- Lingustic analysis:
  - Previous results were from a half-trained model, show results from a fully-trained model.
  - Compare probability of perception embedding sequences (under GPT's language model) to probability of random embeddings.
  - Correlation between particular state attributes and perception embeddings.
    - Not presented today: recording data slows down training too much. We are working on a script that will generate the required data offline after a model is finished training.
- Outperforming randomized parameters:
  - Compare original architecture on one additional Atari environment.
  - Try with more minimal architectures.
...
# ╔═╡ 6a28e7a9-b6ec-4eb8-ba9a-941057bb6a1a
# plot(
# 	umap_coordinates,
# 	x=:x, y=:y,
# 	ygroup=:umap,
...
# 	Guide.colorkey(title="Embedding Source"),
# 	Guide.title("Perception Embeddings vs. GPT Embeddings"),
# 	alpha=[0.5]
# );

# ╔═╡ 0bbac87b-f779-470f-a2b9-d3f1c1a41e10
md"# Linguistic Analysis"

# ╔═╡ 4dc6a513-bf70-40f5-b663-b2e1c8e206d9
md"""
All results below are for `GPT2-Medium` on one `BeamRider` environment trained for `10 million` steps.
"""

# ╔═╡ e68edb38-6667-455d-807b-c2bc6ec62b4e
md"##### Select a Run"

# ╔═╡ 5c9ec017-80f6-4a47-b506-2c4c26758800
@bind la_run_id Select(["4524", "5782", "5789"], default="4524")

# ╔═╡ 64b493c4-2e48-4c5b-b0be-ea82ec8d5dd8
umap_coordinates = CSV.File("/workspace/linguistic_analysis/$la_run_id/umap-coordinates.csv"; types=Dict(:y => Float32)) |> DataFrame |> dropmissing;

# ╔═╡ 374b80ec-44cd-40e6-b20f-9d83d26746f1
# Loads linguistic analysis data
begin
...
# ╔═╡ 552ee197-6f04-4e1a-8a65-8d3cf23c25a0
set_default_plot_size(21cm, 21cm);

# ╔═╡ 3d57725e-3064-4e99-88c7-35a202e74afc
md"#"

# ╔═╡ b5a984d3-c448-4cf9-b2e5-9f2f0924c819
md"""Above is a plot of the distribution of distances between perception embeddings and GPT-2's pretrained embeddings (for each perception embedding, we calculate the distance between it and each GPT-2 embedding) (**blue**). We can see that the distribution is similar to the distribution of distances between random embeddings and the GPT-2 embeddings (**green**).

In addition, note that the distances *among* GPT-2's embeddings (**orange**) are smaller than both the perception distances and the random distances."""

# ╔═╡ 9992cbe7-6b26-4944-b9dd-926aba702a01
pairwise_distance_summary = @chain "/workspace/linguistic_analysis/$la_run_id/pairwise-distance-summary.csv" begin
	CSV.File
	DataFrame
	@aside _.ymin = _.mean - _.std
	@aside _.ymax = _.mean + _.std
	_
end;

# ╔═╡ 3ffdcb7b-c667-421d-b8ca-f842faa70027
md"#"

# ╔═╡ 0649aad0-e268-452d-8cc4-056c3b561698
md"#"

# ╔═╡ eb8d951d-1d88-454d-a139-71955aa05202
md"""
For each observation the perception module produces a sequence of $N$ embeddings (in this case $N = 4$). For each embedding we can find its nearest neighbor among GPT-2's pretrained embeddings (which correspond to tokens), giving us a sequence of tokens. We can then evaluate the probability of this sequence of tokens under GPT-2's language model.

Above is a plot comparing the log-probabilities of the perception module's embedding sequences with sequences of randomly generated embeddings.
"""

# ╔═╡ 52e74f12-f0eb-44e7-9a7c-0a96e298275a
lm_lp_summary = @chain "/workspace/linguistic_analysis/$la_run_id/lm-lp-summary.csv" begin
	CSV.File
	DataFrame
end;

# ╔═╡ e0643700-4527-4ea2-8901-09c4a94274f6
md"# View Samples"

# ╔═╡ 25a2f8dd-644b-4006-9e00-e1566127a027
md"""
This section will allow us to explore individual (Observation, Perception, Action) triples. Use the slider below to explore different samples.
"""

# ╔═╡ 267f2a57-6e58-420c-b523-ca9beb8f7d72
@bind sample_i Slider(1:length(samples); default=1, show_value=true)

# ╔═╡ fb7e306e-a13c-4212-8e45-859ba7d7a26f
cur_sample = samples[sample_i];

# ╔═╡ ac56b79a-1a17-43b3-9fac-40ede516772d
md"# Currently Viewing: Sample $sample_i"

# ╔═╡ 000365d2-6020-41f2-ab33-340a3b10826c
md"##### Observation"

# ╔═╡ 942750d8-196f-48ef-b9c7-180f0cc625a5
md"##### Perception"

# ╔═╡ a52ac047-ffd2-4a87-96ae-3e40b425d08c
md"**Nearest Neighbors**"

# ╔═╡ 64ad2f21-9bd8-4f11-8ed0-3ffdfaed394a
DataFrame(cur_sample.perception_tokens, ["Embedding $i" for i  in 1:length(cur_sample.perception_tokens)])

# ╔═╡ c84df929-aba4-4977-bb01-4ef50b117635
md"##### Action"

# ╔═╡ a6d6e802-9341-42df-a63d-d6d03d8516cc
begin
	set_default_plot_size(21cm, 6cm)
	plot(
	x=["NOOP", "FIRE", "UP", "RIGHT", "LEFT", "UPRIGHT", "UPLEFT", "RIGHTFIRE", "LEFTFIRE"],
	y=cur_sample.actions,
	Geom.bar,
	Guide.xlabel("Action"),
...
plot(
	...
	Scale.color_discrete_manual(
		ling_analysis_colors...;
		levels=["Random", "Perception", "GPT-2"]
	),
	Geom.subplot_grid(Geom.bar(position=:dodge), Geom.yerrorbar, Stat.dodge),
	Guide.xlabel("Quantile"),
	Guide.ylabel("Mean Cosine Distance"),
	Guide.colorkey(title=""),
	Guide.title("Distribution of Distances to GPT-2 Embeddings"),
	Theme(bar_spacing=0mm, stroke_color=c->"black")
) |> HTMLDocument

# ╔═╡ b11a770f-0084-4428-b168-40abcde1985a
# Quantile plot
plot(
	lm_lp_summary,
	x=:quantile, Scale.x_discrete,
	y=:log_prob,
	color=:embedding,
	Scale.color_discrete_manual(ling_analysis_colors...;levels=["Random", "Perception"]),
	Geom.bar(position=:dodge),
	Guide.colorkey(title=""),
	Guide.title("Log-Probability of Embedding Sequences")
) |> HTMLDocument

# ╔═╡ b0966db8-8fed-4943-931e-20a87be9cc98


# Just medians
plot(
	filter(:quantile => ==(0.5), lm_lp_summary),
	x=:embedding, Scale.x_discrete,
	y=:log_prob,
	color=:embedding,
	Scale.color_discrete_manual(ling_analysis_colors...;levels=["Random", "Perception"]),
	Geom.bar(position=:dodge),
	Guide.ylabel("Median Log-Probability"),
	Guide.colorkey(title=""),
	Guide.title("Log-Probability of Embedding Sequences")
) |> HTMLDocument
...
original_architecture_description("Seaquest", 4, 4)
...
# ╔═╡ 4082dea2-b6e2-49fb-9fd6-8df51be02c3f
md"""
# Active runs
- Comparison of best "Two-Convolution, Multi-Embedding" with randomized parameters
- Search for parameters with "One-Convolution, Multi-Embedding"
"""
...
- priming?
- Double check random gpt to verify that it is truly random
...
# ╔═╡ 95ba89c3-9bf7-4d54-a4d1-3309adf7d3a6
to_gif(cur_sample.observation)
...
function plot_returns(;
	...
	data = sweep_runs(sweep_ids, max_step)
	env = first(data)[:env]
