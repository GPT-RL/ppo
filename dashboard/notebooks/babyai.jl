...
md"""
# BabyAI environment overview
- Dynamics and Action Space:
  - Agent navigates $3\times 3$ gridworld (this is adjustable).
  - Agent uses tank controls: left, right, forward, pick-up,  put-down.
  - Agent may only carry one item at a time.
- Obsevation Space:
  - top down view of gridworld with channels encoding
    - object type: box, ball, or key (renamed to phone for Synonyms Environment)
    - color
  - Direction agent is facing (encoded as an integer)
  - Mission: encoded as integer vector using GPT tokenizer
- Reward function:
  - 1 for successful completion of mission
  - 0 all other time steps
...
- In parallel:
  - (a) Feed top-down view of gridworld through a CNN
    - output-size: 32
    - kernel-size: 3
    - stride: 2
  - (b) Encode direction agent is facing as one-hot
  - (c) Encode mission:
    - For PPO baseline: use bag-of-words embedding
...
- Concatenate (a), (b), and (c) and feed through
    ...
    - In parallel:
      ...
      - Project to value estimate with linear layer
"""
...
function mean_and_std(
		df::DataFrame; 
		group_keys::Vector{String}, 
		keep_keys::Vector{String},
		stat_key::String)
	@chain df begin
		groupby(_, group_keys)
		combine(_, [
				stat_key => mean => stat_key, 
				stat_key => std => "$(stat_key)_std",
				[
					name => first => name
					for name in keep_keys
				]...
				])
...
function run_query(query::String, variables::Dict{String, Any}, key::String)
	HASURA_ENDPOINT_URL = "http://rldl12.eecs.umich.edu:1200/v1/graphql"
	@chain query begin
		HTTP.request(
			"POST",
			HASURA_ENDPOINT_URL;
...
# ╔═╡ 8517ee55-970a-4000-b40e-accc76f91ca6
function query(sweep_ids::Vector{Int})
	run_query("""
		query getSweepRuns(\$ids: [Int!]) {
		  run_log(where: {run: {sweep_id: {_in: \$ids}}}) {
			log
			run_id
			run {
			  metadata
...
JSONLines = "24c2b838-1c8b-4567-aca0-d45e60436bb9"
...
[[JSONLines]]
deps = ["CategoricalArrays", "JSON3", "Mmap", "OrderedCollections", "StructTypes", "Tables"]
git-tree-sha1 = "78b3adf109ed44c0b8f1bcbc4a4e100ff16881a4"
uuid = "24c2b838-1c8b-4567-aca0-d45e60436bb9"
version = "2.0.1"
